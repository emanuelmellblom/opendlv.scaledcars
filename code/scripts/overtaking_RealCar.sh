...
#Sensor Proxy
xterm -hold -e "docker run -ti --rm --net=host --ipc=host -e DISPLAY=$DISPLAY --group-add video -v $HOME/DIT-168:/opt/configuration --device=/dev/video0:/dev/video0 seresearch/scaledcars-on-opendlv-on-opendlv-core-on-opendavinci-on-base-dev /opt/opendlv.scaledcars/bin/proxy --cid=111 --freq=20" &
sleep 3
